...
func searchModeIdentifier(input string) string {
	...
	if strings.HasPrefix(input, "エックス") {
		return "X"
	}
...
func Parse(input string) *SearchQuery {
	/*
	   ...
	   次のエリア
	*/
	regex := regexp.MustCompile(`(((次の|前の)*)((\d{0,2}) ?時の)?((ガチマッチ|ガチマ|ガチ|リグマ|バカマ|(レギュラー|リーグ|バンカラ|オープン|チャレンジ|エックス)(マッチ)?)?(ガチ)?(ナワバリ|ナワバリバトル|エリア|ホコ|ホコバトル|ヤグラ|アサリ)?|シャケ|サーモン|サーモンラン|鮭) ?(\d{0,2}))$`)
